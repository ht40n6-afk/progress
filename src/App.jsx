...
function App() {
  ...
  const addGoal = () => {
    ...
    updateData({ ...data, goals: [...data.goals, goalInput.trim()] })
    setGoalInput('')
  }
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <section className="rounded-2xl bg-white p-6 shadow-sm">
            ...
            <div className="mt-4 space-y-4">
              <EntryInput
                ...
                onAdd={() => addListItem('achievements', achievementInput, setAchievementInput)}
                items={todayEntry.achievements}
                xpText="+10 XP each"
              />
              ...
              <EntryInput
                ...
                setValue={setGratitudeInput}
                onAdd={() => addListItem('gratitude', gratitudeInput, setGratitudeInput)}
                items={todayEntry.gratitude}
                xpText="+5 XP each"
              />
              ...
              <EntryInput
                ...
                value={goalNoteInput}
                setValue={setGoalNoteInput}
                onAdd={() => addListItem('goalNotes', goalNoteInput, setGoalNoteInput)}
                items={todayEntry.goalNotes}
                xpText="+15 XP each"
              />
              ...
              <div className="grid grid-cols-2 gap-4">
                <ScoreInput
                  ...
                  onChange={(value) => updateTodayEntry({ ...todayEntry, mood: value })}
                />
                <ScoreInput
                  label="Energy Score"
                  value={todayEntry.energy}
                  onChange={(value) => updateTodayEntry({ ...todayEntry, energy: value })}
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  value={todayEntry.lesson}
                  onChange={(e) => updateTodayEntry({ ...todayEntry, lesson: e.target.value })}
          ...
          <section className="rounded-2xl bg-white p-6 shadow-sm">
            ...
            <div className="mt-4 flex gap-2">
              ...
              />
              <button onClick={addGoal} className="rounded-lg bg-indigo-600 px-4 py-2 font-semibold text-white">Add</button>
            </div>
              ...
              {Object.keys(data.entries)
                ...
                .map((date) => (
                  <div key={date} className="rounded-lg border border-slate-200 p-3">
                    ...
                    <p>Achievements: {data.entries[date].achievements.length}</p>
                    <p>Gratitude: {data.entries[date].gratitude.length}</p>
                    <p>Goal Notes: {data.entries[date].goalNotes.length}</p>
                  </div>
                ))}
...
function EntryInput({ label, value, setValue, onAdd, items, xpText }) {
  return (
    <div>
      <label className="mb-1 block font-medium">{label} ({xpText})</label>
      <div className="flex gap-2">
        ...
        />
        <button onClick={onAdd} className="rounded-lg bg-slate-800 px-3 py-2 text-white">Add</button>
      </div>
...
function ScoreInput({ label, value, onChange }) {
  return (
    <div>
      <label className="mb-1 block font-medium">{label} (1-10)</label>
      <input
        type="range"
        min="1"
        max="10"
...
function sumCount(entries, field) {
  return Object.values(entries).reduce((sum, entry) => sum + entry[field].length, 0)
}

function averageScore(entries, field) {
  const values = Object.values(entries).map((entry) => entry[field]).filter(Boolean)
